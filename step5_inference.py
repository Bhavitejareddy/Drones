...
* A small on-screen panel lists the confidence for every class Stage 1 fired on
  this frame (drone / bird / aircraft / helicopter), and — for any drone that
  cleared the gates — the full nano/micro/small/medium/large tier breakdown
  from Stage 2.
...
# ── Optional extra gates (fall back to sensible defaults if not in config.py) ──
try:
...
# Reuses SMOOTH_FRAMES from config.py (defaults to 20 as per spec).
...
def draw_confidence_panel(frame, panel_entries, stable_label=None):
    ...
    Top-right panel listing, per detected object this frame:
      - Stage-1 class + confidence  (drone / bird / aircraft / helicopter)
      - if it's a drone that cleared the gates: full tier breakdown
        (nano/micro/small/medium/large)
    panel_entries: list of dicts, see _process_frame().
    """
    import cv2
    fh, fw = frame.shape[:2]
    x0 = fw - 260
    ...
    for entry in panel_entries:
        base_label = entry["stage1_label"]
        color = COLORS.get(base_label, DEFAULT_COLOR)
        cv2.putText(frame, f"{base_label}: {entry['stage1_conf']:.2f}",
...
def _process_frame(frame, yolo, classifier, infer_tf, size_classes):
    ...
    Returns list of entries:
      {
        ...
        "stage1_label": str,         # raw Stage-1 class name (drone/bird/...)
        "stage1_conf": float,        # ALWAYS the Stage-1 confidence
    ...
    for box in r.boxes:
        ...
        stage1_conf = float(box.conf[0])
        stage1_label = r.names[cls_id].lower()
...
def _draw_frame(frame, entries, frame_idx, stable_label=None):
    ...
    # Right-side confidence panel (stage1 + stage2 tier breakdown)
    ...
    while cap.isOpened():
        if not paused:
            ...
            if len(frame_label_history) < SMOOTH_WINDOW:
                # Not enough history yet — just show the raw, unsmoothed prediction
                stable_label = dominant
            else:
                # Sliding-window majority vote over the last SMOOTH_WINDOW frames
